...
class StaffAttendance
{
        ...
        */
        ...
        if ($ActiveUser['UserType'] == 2)  //Staff
        {
            ...
            //Marking of  Staff Attendance for Mornig
                ...
                // if()
                // $StaffIn = date('H:i');
                // $getStaffMaxEntryTime = "SELECT * FROM Staff_Attendance_Timing WHERE StaffID=:StaffID";
                // $params4 = array(
                //     array(":StaffID", $ActiveUser['StaffID'])
                // );
                // $resultTime = DBController::sendData($getStaffMaxEntryTime, $params4);
                // if ($resultTime) {
                //     $maxEntryTime = $resultTime['MaximumAllowEntryTime'];
                //     if ($StaffIn > $maxEntryTime) {
                //         return array("return_code" => false, "return_data" => "Your are late for Attemdance.");
                //     } else {
                //         return array("return_code" => true, "return_data" => " Took");
                //     }
                // }
    ...
    */
    ...
    ";
        ...
        return array("return_code" => true, "return_data" => $datas,"TotalCount" => $totalCounts);
    ...
    function getUserAllLeaveOverview($data)
    {
        ...
        FROM Staff_Leave sl 
        INNER JOIN Staff s ON s.StaffID = sl.StaffID 
        WHERE sl.isApproved = 2 AND s.StaffID =:StaffID  AND  MONTH(sl.CreatedDateTime) = MONTH(CURDATE())
        GROUP BY s.StaffID";
    ...
    function getTodaysAttendanceReport()
    {
        ...
        $query = "SELECT sa.AttendanceDate,sa.StaffIn,sa.Status,sa.EntryDateTime,sa.StaffOut, s.StaffName,sa.LatitudeIN ,sa.LatitudeOut ,sa.LongtitudeIN ,am.AttendanceModeID,am.AttendanceMode,sa.LongtitudeOut,sa.AttendancePhotoIN,sa.AttendancePhotoOut,sat.MaximumAllowEntryTime  from Staff_Attendance sa  
        INNER JOIN Staff s on s.StaffID = sa.StaffID
        LEFT join Staff_Attendance_Timing sat  on sat.StaffID  = s.StaffID 
        LEFT JOIN Attendance_Mode am  on am.AttendanceModeID  = sa.AttendanceModeId 
        WHERE sa.AttendanceDate  = DATE(CURDATE())
        ";
        $res = DBController::getDataSet($query);
        if ($res) {
            return array("return_code" => true, "return_data" => $res);
    ...
    function getStaffAttendanceOverview()
    {
        if (isset($_SESSION['StaffID'])) {
            ...
            $sql = "SELECT sa.AttendanceDate,sa.Status,sa.StaffIn,sa.StaffOut,sa.LatitudeIN ,sa.LatitudeOut ,sa.LongtitudeIN ,sa.LongtitudeOut,so.OfficeTimIN,so.OfficeTimeOut  FROM `Staff_Attendance` sa  inner join Staff s  on s.StaffID  = sa.StaffID left join Settings_Office so  on so.OfficeID  =s.OfficeID  where sa.StaffID=:StaffID and MONTH(sa.AttendanceDate) = MONTH(current_timestamp)";
