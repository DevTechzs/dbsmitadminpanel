...
class StaffAttendanceSettings
{
        ...
        if (isset($data['StaffAttendanceTimingID']) && $data['StaffAttendanceTimingID'] != '') {
            ...
            if (isset($data['DesignationID']) &&  $data['DesignationID'] != '') {
                $param8 = array(
                    ...
                    array(":MaximumAllowEntryTime", $data['MaximumAllowEntryTime']),
                    ...
                    array(":IsGPSRequired", $data['IsGPSRequired']),
                    array(":GPSLatititude", isset($data['GPSLatititude']) ? $data['GPSLatititude'] : 0),
                    array(":GPSLongtitude", isset($data['GPSLongtitude']) ? $data['GPSLongtitude'] : 0),
                    array(":AreaInMeter", isset($data['AreaInMeter']) ? $data['AreaInMeter'] : 0),
                );
                $query8 = "UPDATE `Staff_Attendance_Timing` SET `DesignationID`=:DesignationID,`StartTime`=:StartTime,MaximumAllowEntryTime=:MaximumAllowEntryTime,
                `EndTime`=:EndTime,`StaffID`=:StaffID,`AttendanceModeID`=:AttendanceMode,`Supervisor1`=:Supervisor1,`Supervisor2`=:Supervisor2,isGPSRequired=:IsGPSRequired,GPSLatititude=:GPSLatititude,GPSLongtitude=:GPSLongtitude,AreaInMeter=:AreaInMeter WHERE StaffAttendanceTimingID=:StaffAttendanceTimingID";
    ...
    */
    ...
    function getStaffUnderSupervisors()
    {
        $params = array(
            array(":StaffID", $_SESSION['StaffID']) //SuperVisor Login ID
        );
        $query = "SELECT s.StaffID ,s.StaffName  from Staff_Attendance_Timing sat  
left join Staff s on s.StaffID  = sat.StaffID 
where sat.Supervisor1  =:StaffID or sat.Supervisor2 =:StaffID AND s.isRemoved = 0 and  sat.Supervisor1 =:StaffID OR sat.Supervisor2 =:StaffID";
        $res = DBController::getDataSet($query, $params);
        return array("return_code" => true, "return_data" => $res);
    }
}
