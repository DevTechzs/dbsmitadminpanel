    ...
    function loadTodaysAttendance(data) {
        ...
        if (data.length == 0) {
            ...
            for (let i = 0; i < data.length; i++) {
                text += '<tr> ';
                var MaximumAllowEntryTime = data[i].MaximumAllowEntryTime;
                var EntryTime = data[i].EntryDateTime.split(' ')[1]; // Assuming EntryDateTime is in format 'YYYY-MM-DD HH:MM:SS'

                // Convert times to Date objects for comparison
                var maxEntryTimeDate = new Date('1970-01-01T' + MaximumAllowEntryTime + 'Z');
                var entryTimeDate = new Date('1970-01-01T' + EntryTime + 'Z');

                // Calculate the time difference in minutes
                var diffMs = entryTimeDate - maxEntryTimeDate;
                var diffMins = Math.floor((diffMs % 86400000) / 60000);
                var diffHours = Math.floor(diffMins / 60);
                var remainingMins = diffMins % 60;
                text += '<td> ' + data[i].StaffName + '<p style="font-size: 12px; color:black;">(' + data[i].AttendanceMode + ')<p/><p style="font-size: 13px; color: black;">Average Entry Timing:(' + data[i].MaximumAllowEntryTime + ' am)</p></td>';
                ...
                if (entryTimeDate > maxEntryTimeDate) {
                    var lateEntryText = +diffHours + ' hours ' + remainingMins + ' minutes late';
                    text += '<td><p style="font-size: 13px; color: red;">' + lateEntryText + '</p></td>';
                } else {
                    text += '<td> <span class="badge custombadge-success">Not Late</span> </td>';
                }
