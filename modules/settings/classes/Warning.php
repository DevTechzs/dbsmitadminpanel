...
class Warning
{
    ...
    function getStaffUnderSupervisor()
    {
        $param = array(
            array(":Supervisor1ID", strip_tags($_SESSION['UserID'])),
            array(":Supervisor2ID", strip_tags($_SESSION['UserID'])),
        );
        $query = "SELECT s.StaffID ,s.StaffName 
FROM Staff s
inner JOIN Staff_Attendance_Timing sat
ON s.staffID = sat.staffID WHERE  sat.Supervisor1=:Supervisor1ID OR sat.Supervisor2=:Supervisor2ID";
        $res = DBController::getDataSet($query, $param);
        if ($res)
            return array("return_code" => true, "return_data" => $res);
        else
            return array("return_code" => false, "return_data" => "No  staff under supervisor");
    }
}
